...
export async function handleCreateMention(authorHandle: string, tweetId: string) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://clone.fun';
  ...
  if (xUser.description) {
    ...
    if (isCompany) {
      await sendTweet(
        `@${authorHandle} clone is designed for individual creators, not company or brand accounts. If you're a person behind this account, update your bio and try again!`,
        tweetId
      );
      return;
    }
